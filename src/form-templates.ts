...
export function getFormTemplate(formName: string): FormTemplate | null {
...
// Add form selection logic at the end of the file
...
export function analyzeUserIntent(context: FormSelectionContext): FormSuggestion[] {
...
export function selectBestForm(context: FormSelectionContext): FormSuggestion | null {
    const suggestions = analyzeUserIntent(context);
    return suggestions.length > 0 ? suggestions[0] : null;
}

export function getFormSuggestions(userInput: string, maxSuggestions: number = 3): FormSuggestion[] {
    const context: FormSelectionContext = { userInput };
    const suggestions = analyzeUserIntent(context);
    return suggestions.slice(0, maxSuggestions);
}

// Resource template for dynamic form queries
export function createFormResourceTemplate(formId: string, parameters?: Record<string, any>): string {
    const template = getFormTemplate(formId);
    if (!template) {
